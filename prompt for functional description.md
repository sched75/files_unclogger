...
- **Flexibilité maximale** : 8 templates d'organisation adaptés à tous les besoins minimum
...
il faudrait que l'ia repere les fichiers de données de meme structure. ajoute aussi des types de documents comme "code", "synopsis","analyse",etc... et prévoit de pouvoir classer des fichiers multimedias audio et video en recuperant leurs titres et autres données.
fait la modification en conservant tout le reste du fonctionnement deja décrit.

process de nettoyage des fichiers
1-Selectionner la racine des fichiers et repertoires à ranger.
2-Afficher un ecran avec une arborescence des fichiers et repertoires à gauche et à droite la liste des topics, la liste des projets, la liste des templates de classement. Une colonne de l'arborescence affiche le topic associé, le projet associé, le template de classement, le repertoire de destination.
3-quand on glisse-depose un topic ou un projet ou un template sur un fichier celui-ci est mis à jour et toute l'attribution automatique à partir du repertoire modifié est réévalué par l'ia.

le document doit être modifié pour adapter le look and feel, les configurations, les exemples de use cases etc...

ajoute au document qu'il faut que l'utilisateur puisse attribuer un fichier donné à un mode de classement et à un sujet. par exemple pour un rapport de comptabilité, le topic sera Comptabilité/Point cash et contiendra un classement des fichiers en mode année/mois.
L'utilisateur doit pouvoir definir sa liste de topic comme il peut definir sa liste de projets.
Il faut également pouvoir definir pour les topics et les projets les repertoires racine par defaut.
le document ne doit aborder aucun aspect relatif à l'architecture technique ou au codage.

il faut au moins 8 templates prédéfinis d'organisation et la possibilité d'en creer d'autres : 
* {reccurent}/{Domaine,Sous-Domaine}/{Sujet,Projet}/{Année,FY}/{Mois}
* {reccurent}/{Domaine,Sous-Domaine}/{Sujet,Projet}/{Année}/{Semaine}
* {reccurent}/{Domaine,Sous-Domaine}/{Sujet,Projet}/{Année,FY}/{Mois}/{Type}
* {reccurent}/{Domaine,Sous-Domaine}/{Sujet,Projet}/{Année}/{Semaine}/{Type}
* {en cours,terminé,archivé,reccurent}/{Domaine,Sous-Domaine}/{Projet}
* {en cours,terminé,archivé,reccurent}/{Domaine,Sous-Domaine}/{Projet}/{Preparation,Documentation,Faisabilité,Plannification,ROI,Suivi}/{Lot}/{Budget,Planning,Ressources,Recherches,Suivi,Réalisation,Tests,Deploiement,VSR}

ajoute la possibilité de definir des phases projets pour chaque projet.

le programme doit permettre de centraliser les regles pour tous les utilisateurs et de les gerer pour toute l'entreprise.

le programme ne doit servir qu'a organiser des ensembles de données mal organisées mais les un autre programme doit permettre de maintenir l'organisation des données.

tout est mis en oeuvre pour garantir la preserevation des données lors de la reorganisation.

En aucuns cas le document ne doit aborder les questions d'architecture techniques et de codage.
